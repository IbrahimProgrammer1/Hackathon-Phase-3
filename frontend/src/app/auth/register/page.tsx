...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl.replace(/\/$/, '')}/api/auth/register`, {
        method: 'POST',
      ...
      if (!res.ok) {
        const msg = data?.error || data?.detail || `Registration failed (${res.status})`;
        throw new Error(msg);
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--bg)] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-[var(--text)] mb-2">Create your NAFAY account</h1>
          <p className="text-[var(--muted)]">Organize your tasks. Focus on what matters.</p>
        </div>
        ...
        <div className="card p-8">
          <form className="space-y-6" onSubmit={handleRegister}>
            ...
            <div className="space-y-4">
              <div className="form-group">
                <label htmlFor="full-name" className="block text-sm font-medium text-[var(--text)] mb-2">
                  Full Name
                </label>
                <input
                  id="full-name"
              ...
              <div className="form-group">
                ...
                <p className="text-xs text-[var(--muted)] mt-2">At least 8 characters</p>
              </div>
            </div>
          ...
          <div className="mt-6 text-center text-sm text-[var(--muted)]">
            ...
            <a href="/auth/login" className="font-semibold text-[var(--primary)] hover:text-[var(--primary)]/80 transition-colors">
              Sign in
            </a>
          </div>
        </div>
